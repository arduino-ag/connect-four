...
constexpr int ENTER = 6;
constexpr int RIGHT = 5;
constexpr int START = 4;
constexpr int RESET = 3;

constexpr int LED_PIN = 11;
constexpr int LED_COUNT = ROW_COUNT * COLUMN_COUNT;

constexpr int CELL_COUNT = ROW_COUNT * COLUMN_COUNT;
...
boolean areFourConnected(State player) {
  ...
  for (int j = 0; j < ROW_COUNT - 3; j++) {
    for (int i = 0; i < COLUMN_COUNT; i++) {
      if (get_board(i, j) == player && get_board(i, j + 1) == player &&
          get_board(i, j + 2) == player && get_board(i, j + 3) == player) {
        return true;
      }
    }
  }
  ...
  for (int i = 0; i < COLUMN_COUNT - 3; i++) {
    for (int j = 0; j < ROW_COUNT; j++) {
      if (get_board(i, j) == player && get_board(i + 1, j) == player &&
          get_board(i + 2, j) == player && get_board(i + 3, j) == player) {
        return true;
      }
    }
  }
  ...
  for (int i = 3; i < COLUMN_COUNT; i++) {
    for (int j = 0; j < ROW_COUNT; j++) {
      if (get_board(i, j) == player && get_board(i - 1, j + 1) == player &&
          get_board(i - 2, j + 2) == player &&
          get_board(i - 3, j + 3) == player)
        return true;
    }
  }
...
State get_board(int x, int y) { return game[x + y * COLUMN_COUNT]; }
